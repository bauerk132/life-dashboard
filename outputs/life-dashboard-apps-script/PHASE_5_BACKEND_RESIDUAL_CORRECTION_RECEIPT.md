...
| `outputs/life-dashboard-apps-script/tests/scoring.test.js` (1052 lines) | Migrated Suites 1–5 for the new dispatch split and strict grounding; added Suite 6 (B1–B6 direct coverage, 21 tests, across both the implementation commit and the follow-up negative-`thoughtsTokenCount` addition); T03 replaced with a rejection test (see Section 5). |
...
- `node --test tests/scoring.test.js`: **49 pass / 0 fail** (8 suites; Suite 6 "Residual correction B1-B6" is new, 21 tests covering orphan-reservation elimination, the exact input-limit boundary, conservative-vs-actual charging, ledger fail-closed aggregation, and every schema/grounding rejection path).
